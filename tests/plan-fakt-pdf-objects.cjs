...
async function pixels(f,ids=[],crop=false){const c=canvas.createCanvas(crop?400:f.vp.width,crop?300:f.vp.height),ctx=c.getContext('2d');await engine.render(f.page,f.scene,new Set(ids),{canvasContext:ctx,viewport:crop?f.page.getViewport({scale:8}):f.vp,transform:crop?[1,0,0,1,-600,-500]:undefined,background:'#fff'},lib);return ctx.getImageData(0,0,c.width,c.height).data;}
const bg='q .8 .9 .7 rg 0 0 200 160 re f Q\n',blue='0 0 1 RG 3 w ',h='20 80 m 180 80 l ',v='100 20 m 100 140 l ';
async function compare(name,source,expected,select,rotation=0){const a=await fixture(bg+source,rotation),b=await fixture(bg+expected,rotation);const before=JSON.stringify(a.scene.ol),ids=select(a.scene.objects).map(o=>o.id);assert(ids.length,name+' target');const actual=await pixels(a,ids),want=await pixels(b);assert.deepEqual(actual,want,name);assert.deepEqual(await pixels(a,ids,true),await pixels(b,[],true),name+' high-resolution crop');assert.equal(JSON.stringify(a.scene.ol),before,'operator list immutable');assert.deepEqual(await pixels(a),await pixels(a,[]),'undo repeat deterministic');await a.pdf.destroy();await b.pdf.destroy();console.log('PASS',name);}
(async()=>{
