# Текущее состояние · обновлено 2026-08-24
...
| Дата | Commit | Что |
|---|---|---|
| 2026-08-24 | `6b5df74` | fix(calendar): кнопка «Открыть сделку» работает для Meta-лидов без Bitrix ID; PWA принудительно обновляется |
| 2026-08-24 | `9b57003` | fix(elc): старая PWA сохраняет Zoom и другие поля в только что восстановленную Meta-сделку |
| 2026-08-24 | `332e4e8` | fix(crm): сервер восстанавливает точный Meta deal ID для шапки и Zoom даже у старой PWA |
| 2026-08-24 | `65b3bae` | fix(crm): Service Worker v3 принудительно обновляет старые PWA с `deal_null`; отклонённые старые записи журналируются |
| 2026-08-23 | `cb6bd74` | fix(crm): стабильные ID Meta-лидов, сохранение примечаний и календарных дат, автообновление долго открытой CRM и запрет ложного успеха Worker |
| 2026-08-15 | `f52901a` | fix(calendar): календарные поля и действия карточки Meta-сделки используют стабильный внутренний ID вместо `deal_null` |
| 2026-08-15 | `f160b65` | fix(calendar): Ирина может создавать обычные события и дела из Meta-сделок без `bitrix_id`; единый `/api/tasks` и аудит |
| 2026-08-15 | `b0ef34a` | fix(elc): сотрудники с правом «Все сделки» могут менять стадии неназначенных лидов в доступной воронке |
| 2026-08-11 | — | feat(meta): прием лидов формы «3 Вопроса/Высокое намерение» в ELC Almaty / NEW |
| 2026-08-10 | — | fix(calendar): синхронизация поля названия сделки в задачах календаря |
| 2026-08-10 | — | fix(whatsapp): резервное создание сделок из новых диалогов без синхронизированного текста |
