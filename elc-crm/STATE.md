...
- Исправление создания календарных событий Ириной: единый POST `/api/tasks` для общего календаря и карточки сделки; привязка к внутреннему `deals.id` для Meta-лидов без `bitrix_id`; аудит создания задач.
