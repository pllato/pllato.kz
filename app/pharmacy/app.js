...
function contractorModal(r){
  ...
  <div class="modal-b">
    ...
    <div class="panel section-gap" style="margin-top:14px"><div class="panel-h" style="flex-wrap:wrap;gap:8px"><h3>${ic('i-money','sm')} История покупок · 1С</h3><div style="margin-left:auto;display:flex;align-items:center;gap:6px"><input type="date" id="cmFrom" class="sel" style="padding:4px 7px;font-size:12px" title="С даты"><span class="muted2">—</span><input type="date" id="cmTo" class="sel" style="padding:4px 7px;font-size:12px" title="По дату"><button class="btn sm" id="cmHistReset" title="Сбросить период">${ic('i-x','sm')}</button><span class="ph-sub" id="cmHistSub">загрузка…</span></div></div>
...
function loyaltyBannerHtml(L){
  ...
  return `<div class="loy-banner lv${lvl}">
    ...
    ${(L.points!=null&&(L.has_card||L.points>0))?`<div class="lb-pts">${ic('i-gift','sm')} Баллов на карте: <b>${(L.points||0).toLocaleString('ru-RU')}</b></div>`:''}
...
async function loadContractorHistory(r){
  ...
  const fromI=document.getElementById('cmFrom'), toI=document.getElementById('cmTo'), rstB=document.getElementById('cmHistReset');
  if(fromI) fromI.onchange=()=>loadContractorHistory(r);
  if(toI) toI.onchange=()=>loadContractorHistory(r);
  if(rstB) rstB.onclick=()=>{ if(fromI)fromI.value=''; if(toI)toI.value=''; loadContractorHistory(r); };
  const from=fromI&&fromI.value?fromI.value:'', to=toI&&toI.value?toI.value:'';
  const res=await api('/api/1c/sales?contractor='+encodeURIComponent(r.ref_key)+'&limit=200'+(from?'&from='+from:'')+(to?'&to='+to:''));
...
PAGES.sales=async(c)=>{
  ...
  function sellersHTML(rows){
    ...
    const body=rows.length?rows.map(s=>{ const pct=Math.round((s.revenue||0)/total*100);
      return `<tr><td><div class="cell-name"><span class="avatar-xs" style="background:${avBg(s.name||s.key||'?')}">${initials(s.name||'П')}</span><div>${esc(s.name||('продавец '+String(s.key||'').slice(0,8)))}</div></div></td><td class="num">${money(s.revenue||0)}</td><td class="num">${money(s.profit||0)}</td><td class="num">${fmtN(s.qty)}</td><td class="num"><div style="display:flex;align-items:center;gap:6px;justify-content:flex-end"><span style="min-width:30px">${pct}%</span><div style="width:44px;height:6px;background:var(--line);border-radius:3px;overflow:hidden;flex:none"><div style="width:${pct}%;height:100%;background:var(--accent)"></div></div></div></td></tr>`; }).join(''):'<tr><td colspan="5" class="muted2" style="padding:16px">Нет данных</td></tr>';
    return `<div class="panel"><div class="panel-h"><h3>${ic('i-target','sm')} KPI продавцов</h3><span class="ph-sub" style="margin-left:auto">доля — % от факта продаж</span></div><table class="tbl"><thead><tr><th>Продавец</th><th class="num">Выручка</th><th class="num">Прибыль</th><th class="num">Позиций</th><th class="num">Доля</th></tr></thead><tbody>${body}</tbody></table></div>`;
  }
...
function actionValueLabel(t){ return ({percent:'Скидка, %',fixed:'Скидка, сом','1plus1':'Купи N (1 = «1+1»)',bundle:'Скидка на набор, %',gift:'Порог суммы, сом (0 = любая)'})[t]||'Параметр'; }
function actionTypeHint(t){ return ({
  percent:'Скидка N% на товары из области ниже.',
  fixed:'Фиксированная скидка (сом) на товары из области.',
  '1plus1':'Купи N шт из области — 1 в подарок (для «1+1» поставь N=1). Бесплатным считается самый дешёвый товар.',
  bundle:'Купи товары набора (область ниже) вместе — скидка N% на набор. Состав набора опиши в комментарии.',
  gift:'Покупка из области на сумму ≥ порога → подарок. Что именно дарим — опиши в комментарии.',
})[t]||''; }
...
async function newActionLive(onSaved){
  ...
  <div class="modal-b">
    ...
    <div class="note blue" data-na="vhint" style="margin:0 0 4px;font-size:11.5px"></div>
  ...
  const typeSel=bg.querySelector('[data-na=type]'), vlabel=bg.querySelector('[data-na=vlabel]'), vhint=bg.querySelector('[data-na=vhint]');
  const upd=()=>{ vlabel.textContent=actionValueLabel(typeSel.value); if(vhint)vhint.innerHTML=ic('i-info','sm')+' '+actionTypeHint(typeSel.value); };
...
async function actionModalLive(a,onSaved){
  ...
  <div class="modal-b">
    ...
    <div class="note blue" data-am="vhint" style="margin:0 0 4px;font-size:11.5px"></div>
  ...
  const typeSel=bg.querySelector('[data-am=type]'), vlabel=bg.querySelector('[data-am=vlabel]'), vhint=bg.querySelector('[data-am=vhint]');
  const updH=()=>{ vlabel.textContent=actionValueLabel(typeSel.value); if(vhint)vhint.innerHTML=ic('i-info','sm')+' '+actionTypeHint(typeSel.value); };
  typeSel.onchange=updH; updH();
...
async function doctorProfitModal(d,rate,period){
  ...
  body.innerHTML=`<div class="cards-row" style="margin-bottom:12px">
    ...
    (rows.length?`<table class="tbl dp-table"><colgroup><col><col style="width:48px"><col style="width:92px"><col style="width:88px"><col style="width:92px"><col style="width:94px"><col style="width:120px"></colgroup><thead><tr><th>Товар</th><th class="num">Кол</th><th class="num">Выручка</th><th class="num">Скидка</th><th class="num">Себест.</th><th class="num">Кэшбэк</th><th class="num">Маржа</th></tr></thead><tbody>`+
      rows.map(x=>`<tr><td class="dp-name" title="${esc(x.name||'')}">${esc(x.name||'—')}</td><td class="num">${(x.qty||0).toLocaleString('ru-RU')}</td><td class="num">${money(x.revenue)}</td><td class="num muted2">${money(x.disc)}</td><td class="num muted2">${x.unit_cost?money(x.cost):'<span title="нет закупочной цены в 1С">—</span>'}</td><td class="num">${money(x.cb)}</td><td class="num${x.unit_cost?'':' muted2'}"${(x.unit_cost&&x.margin<0)?' style="color:#dc2626;font-weight:600"':''}${x.unit_cost?'':' title="без закупочной цены — маржа неполная"'}>${money(x.margin)} <span class="muted2" style="font-weight:400">${x.unit_cost?x.mp+'%':'?'}</span></td></tr>`).join('')+
...
function greenApiPanel(){
  const panel = el(`<div class="panel section-gap" style="margin-top:0">
    ...
    <div class="panel-b" data-ga="body" style="display:none">
      ...
      <label class="ck" style="display:flex;align-items:center;gap:9px;font-size:13px;color:var(--txt);cursor:pointer;margin-top:16px;padding-top:14px;border-top:1px solid var(--line)">
        <input type="checkbox" data-ga="autolead"> Авто-лид: входящее сообщение создаёт сделку
        <span class="muted2" style="font-size:11px">— новый/повторный контакт → сделка «Заявка» в воронке номера</span></label>
      <label class="ck" style="display:flex;align-items:center;gap:9px;font-size:13px;color:var(--txt);cursor:pointer;margin-top:10px">
        <input type="checkbox" data-ga="audio"> Разрешить отправку аудиосообщений (голосовых) клиентам
        <span class="muted2" style="font-size:11px">— включит микрофон в чате</span></label>
    </div></div>`);
  ...
  const audioCb=panel.querySelector('[data-ga=audio]'), leadCb=panel.querySelector('[data-ga=autolead]');
  api('/api/admin/greenapi/status').then(r=>{ if(r&&r.ok&&r.data){ if(audioCb)audioCb.checked=!!r.data.allow_audio; if(leadCb)leadCb.checked=!!r.data.autolead; } });
  if(audioCb) audioCb.onchange=async()=>{ const r=await api('/api/admin/greenapi/settings',{method:'PUT',body:JSON.stringify({allow_audio:audioCb.checked})}); toast(r.ok?(audioCb.checked?'Аудио включено — в чате появится микрофон':'Аудио отключено'):'Ошибка','i-'+(r.ok?'check2':'x'),r.ok?'':'#dc2626'); };
  if(leadCb) leadCb.onchange=async()=>{ const r=await api('/api/admin/greenapi/settings',{method:'PUT',body:JSON.stringify({autolead:leadCb.checked})}); toast(r.ok?(leadCb.checked?'Авто-лид включён — входящие создают сделки':'Авто-лид выключен'):'Ошибка','i-'+(r.ok?'check2':'x'),r.ok?'':'#dc2626'); };
