...
function seed(){
  ...
  const audit=[
    ...
    ['30.07 · 18:12','Айжан Темирова','Проверила предоплату 6 400 000 ₸','P-1050'],
    ['30.07 · 16:30','Азиз Исмаилов','Утвердил индивидуальную расценку монтажа 3 600 ₸/м²','—']
  ...
  return {v:3,users,projects,materials,rates,accruals,audit,checks,selectedProject:'P-1050'};
}

const KEY='okna_aziz_spec_demo_v3';
function load(){ try{ const d=JSON.parse(localStorage.getItem(KEY)||'null'); if(d&&d.v===3)return d; }catch(e){} const d=seed(); localStorage.setItem(KEY,JSON.stringify(d)); return d; }
let DB=load();
  ...
  return `${head('Пульт директора','Вся компания в одном рабочем экране','<button class="btn dark" data-action="nav" data-module="coverage">Проверить ТЗ · 33/33</button>')}
    ...
    <div class="dash-bottom">
      <div class="surface"><div class="surface-head"><h3>Карта работ</h3><span class="sub">объекты и сотрудники в поле</span><span class="spacer"></span><button class="btn small" data-action="nav" data-module="map">Открыть карту</button></div>
        <div class="mini-map"><span class="map-label">Алматы · 8 активных точек</span><i class="map-route" style="left:30%;top:42%;width:35%;transform:rotate(18deg)"></i>${DB.projects.map((p,i)=>`<button class="marker ${i===2?'pulse':''}" style="left:${p.lat}%;top:${p.lon}%;--c:${i===2?'var(--red)':'var(--green)'}" data-action="project" data-id="${p.id}">${i+1}</button>`).join('')}</div>
      </div>
...
function reworkView(){
  return `${head('Переделки и дефекты','Отдельный контур причины, ответственности и стоимости')}
    <div class="surface table-wrap"><table><thead><tr><th>Проект</th><th>Проблема</th><th>Причина</th><th>Виновный этап</th><th>Ответственный</th><th>Повторный объём</th><th class="num">Стоимость</th><th>Срок</th></tr></thead><tbody>
      <tr><td class="project-id">P-1048</td><td><strong>Царапина на стеклопакете</strong><br><span class="muted">3 фото приложено</span></td><td>Повреждение при сборке</td><td>${label('Производство','red')}</td><td>Бауыржан Омаров</td><td>2,8 м²</td><td class="num">68 000 ₸</td><td>${label('02.08','amber')}</td></tr>
      <tr><td class="project-id">P-1037</td><td><strong>Регулировка створки</strong><br><span class="muted">акт сервисного выезда</span></td><td>Монтажная настройка</td><td>${label('Монтаж','amber')}</td><td>Бригада №2</td><td>1 выезд</td><td class="num">18 000 ₸</td><td>${label('Закрыто','green')}</td></tr>
    </tbody></table></div>
