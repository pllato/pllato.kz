...
/* ============ SEED BUILDER ============ */
function buildSeed(){
  const company = { name:'Окна Сервис', legal:'ОсОО «Окна Сервис»', city:'Бишкек', phone:'+996 555 44-22-11',
    workshop:'оконный завод · 11 отделов · 42 сотрудника', revenueYear:'демонстрационные данные' };

  const users = [
    {id:'u_isk', name:'Азиз Исмаилов',   role:'director',   title:'Директор',                 primary:true},
    {id:'u_pm',  name:'Айпери Токтосунова',role:'manager',  title:'Менеджер отдела продаж',   primary:true},
    {id:'u_ps',  name:'Руслан Абдраев',  role:'surveyor',   title:'Замерщик',                 primary:true},
    {id:'u_fin', name:'Алина Жумабаева', role:'finance',    title:'Финансовый контролёр',     primary:true},
    {id:'u_head',name:'Нурбек Эсенов',   role:'head',       title:'Руководитель производства',primary:true},
    {id:'u_buy', name:'Бакыт Омуралиев', role:'procurement',title:'Снабженец',                primary:false},
    {id:'u_qc',  name:'Эльмира Кадырова',role:'quality',    title:'Инспектор ОТК',            primary:false},
    {id:'u_log', name:'Тимур Асанов',    role:'logistics',  title:'Логист',                   primary:false},
    {id:'u_as',  name:'Бауыржан Омаров', role:'production', title:'Сборщик',                  primary:false},
    {id:'u_wh',  name:'Марат Ким',       role:'warehouse',  title:'Заведующий складом',       primary:false},
  ];
...
/* ============ STATE / PERSISTENCE ============ */
const DB_KEY = 'okna_service_crm_full_v1';
let DB;
function loadDB(){
...
const THEME_KEY = 'okna_service_crm_theme';
...
/* ============ PERMISSIONS ============ */
const MODULE_ROLES = {
  dashboard:['director','manager','head'],
  funnel:   ['director','manager'],
  clients:  ['director','manager'],
  measure:  ['director','manager','surveyor'],
  project:  ['director','manager','surveyor','finance','head','procurement','quality','logistics','production','warehouse'],
  objectmap:['director','surveyor','logistics'],
  procurement:['director','head','procurement','warehouse'],
  warehouse:['director','head','procurement','warehouse','production'],
  production:['director','head','production','warehouse','surveyor'],
  quality:['director','head','quality','production'],
  logistics:['director','head','logistics','surveyor'],
  payroll:['director','finance','head','manager','surveyor','production','warehouse','procurement','quality','logistics'],
  finance:  ['director','finance','manager'],
  audit:    ['director','finance','head'],
  settings: ['director'],
};
function canSee(mod){ return state.user && MODULE_ROLES[mod] && MODULE_ROLES[mod].includes(state.user.role); }
function seesMoney(){ return state.user && ['director','finance','manager'].includes(state.user.role); }
