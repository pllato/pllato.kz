...
document.addEventListener('click', e=>{
  ...
  switch(a){
    ...
    case 'os-check': osToggleCheck(t.dataset.i); break;
    case 'os-next': osAdvanceStage(); break;
    case 'os-object': {
      const o=DB.os.objects.find(x=>x.id===id);
      if(o) openModal(`<div class="modal-h">${icon('pin')}<div><h3>${o.id} · ${o.name}</h3><div class="mh-sub">${o.stage}</div></div><button class="x" data-act="close-modal">${icon('x')}</button></div><div class="modal-b"><div class="os-facts"><div><span>Ответственный</span><b>${o.who}</b></div><div><span>Срок</span><b>${o.due}</b></div><div><span>Проблема</span><b>${o.kind==='late'?'Отставание от плана':'Нет'}</b></div><div><span>Следующее действие</span><b>${o.kind==='late'?'Перепланировать производство':'Следовать чек-листу'}</b></div></div></div><div class="modal-f"><button class="btn" data-act="close-modal">Закрыть</button><button class="btn primary" data-act="os-route" data-id="${o.id}">Построить маршрут ↗</button></div>`);
    } break;
    case 'os-route': toast('Маршрут построен · 18 минут · пробки 2 балла'); break;
    case 'os-purchase': osCycle(DB.os.purchases,id,['Срок подтверждён','На загрузке','В пути','Прибыл']); break;
    case 'os-qc': toast('Акт ОТК открыт: чек-лист, фото и ответственный за исправление'); break;
    case 'os-trip': osCycle(DB.os.trips,id,['Назначен','На загрузке','В пути','На объекте','Завершён']); break;
    case 'os-approve': {
      const x=DB.os.accruals[Number(t.dataset.i)];
      if(x){ x.status='Подтверждено руководителем'; saveDB(); renderModule(); toast('Объём подтверждён и передан финансовому отделу'); }
    } break;
    case 'os-period': {
      if(!['director','finance'].includes(state.user.role)) break;
      const seq=['Открыт','Сбор данных','Подтверждение руководителей','Проверка финансов','Утверждение директора','Закрыт'];
      const i=seq.indexOf(DB.os.payroll.status);
      DB.os.payroll.status=seq[Math.min(seq.length-1,i+1)];
      saveDB(); renderModule(); toast('Статус расчётного периода обновлён');
    } break;
