...
function snapshot(){const live=S._pdfScene,src=source(),pn=S.pageNum;if(!src||!live||live.src!==src||live.pn!==pn||S.vectorLayer||!S.W||!S.H||!S.scale||!baseimg.complete||!vp.clientWidth||!vp.clientHeight)return null;
  const scene=live.scene,z=S.scale,margin=32/z,x=Math.max(0,-S.pan.x/z-margin),y=Math.max(0,-S.pan.y/z-margin),right=Math.min(S.W,(vp.clientWidth-S.pan.x)/z+margin),bottom=Math.min(S.H,(vp.clientHeight-S.pan.y)/z+margin),w=right-x,h=bottom-y;if(w<=0||h<=0)return null;
