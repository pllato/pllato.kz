...
const ROLES={
 ...
 'Управляющий филиалом':{av:'УФ',n:'Асель · Астана, Мангилик Ел',r:'свой филиал',note:'Ученики, абонементы, посещения, оплаты и должники своего филиала, тренеры и их часы, переводы, расписание, воронка филиала',
...
/* ====== ФИЛИАЛЫ ====== */
const BR=[
 {id:'B01',city:'Астана',n:'Мангилик Ел 52',brand:'Grand Dance',mgr:'Асель',st:148,pay:117,rev:4020000,plan:4300000,tr:5,c:'#8c1d3f'},
 {id:'B02',city:'Астана',n:'Туран 37',brand:'Grand Dance',mgr:'Мадина',st:132,pay:108,rev:3680000,plan:3800000,tr:4,c:'#8c1d3f'},
 {id:'B03',city:'Астана',n:'Сыганак 18',brand:'Ритм',mgr:'Жанар',st:96,pay:75,rev:2460000,plan:2800000,tr:3,c:'#b2452b'},
 {id:'B05',city:'Астана',n:'Кабанбай батыра 11',brand:'Pas de Deux',mgr:'Динара',st:88,pay:72,rev:2500000,plan:2500000,tr:3,c:'#5b2a86'},
 {id:'B04',city:'Алматы',n:'Розыбакиева 247',brand:'Grand Dance',mgr:'Сауле',st:124,pay:98,rev:3480000,plan:3500000,tr:4,c:'#8c1d3f'},
...
const multiCity=c=>BR.filter(x=>x.city===c).length>1;
...
const TRANS=[
 {id:'ПР-14',stu:'S-13',from:'B04',to:'B01',d:'2026-09-22',st:'wait',reason:'переезд семьи в Астану',sub:'AY',paid:280000,used:48,les:96,start:'2026-03-01'},
...
const LEADS=[
 ...
 {id:'L-502',n:'Ева, 8 лет',par:'Ольга',ph:'+7 777 ··· 90 12',src:'2ГИС → WhatsApp',ad:'карточка «Мангилик Ел 52»',br:'B01',k:'call',d:'2026-09-23',next:'перезвонить 18:00, выбирает дни'},
 {id:'L-503',n:'Адиль, 11 лет',par:'Марат',ph:'+7 701 ··· 33 20',src:'Facebook',ad:'лид-форма «спортивные бальные»',br:'B01',k:'new',d:'2026-09-24',next:'бот ответил, ждёт менеджера'},
 ...
 {id:'L-506',n:'Зере, 6 лет',par:'Айгерим',ph:'+7 771 ··· 42 19',src:'2ГИС → WhatsApp',ad:'карточка «Розыбакиева 247»',br:'B04',k:'trial',d:'2026-09-23',next:'пробное 25.09 16:00'},
...
const TODAYL=[
 ['r','Тренер Дмитрий Волков (Мангилик Ел) не вышел 4-й раз за две недели: 8 занятий группы G-03 не проведены, 10 учеников. Оплата за них не начислена. Решение: замена или увольнение','card(\'trainer\',\'T-03\')','Тренер'],
 ...
 ['w','Перевод ПР-14: Мадина Кожахметова из Алматы в Астану, годовой абонемент: использовано 48 из 96, остаток 140 000 ₸ переходит на Мангилик Ел','card(\'transfer\',\'ПР-14\')','Перевод'],
...
const brFilter=()=>`<div class="ptabs"><button class="ptab ${curBr==='all'?'on':''}" onclick="curBr='all';render()">Вся сеть</button>${BR.map(b=>`<button class="ptab ${curBr===b.id?'on':''}" onclick="curBr='${b.id}';render()" title="${esc(b.brand)}">${esc(b.city)}${multiCity(b.city)?' · '+esc(b.n.split(' ')[0]):''}</button>`).join('')}</div>`;
...
SC.funnel=()=>{const leads=LEADS.filter(l=>funBr==='all'||l.br===funBr);const cnt=k=>leads.filter(l=>l.k===k).length;const NETF={new:64,call:48,trial:37,came:26,sub:19};
 ...
 <div class="ptabs"><button class="ptab ${funBr==='all'?'on':''}" onclick="funBr='all';render()">Вся сеть</button>${BR.slice(0,6).map(b=>`<button class="ptab ${funBr===b.id?'on':''}" onclick="funBr='${b.id}';render()">${esc(b.city)}${multiCity(b.city)?' · '+esc(b.n.split(' ')[0]):''}</button>`).join('')}<button class="ptab" onclick="funBr='B11';render()">Усть-Каменогорск</button></div>
...
SC.parent=()=>{const s=S_('S-01');const t=ST_(s.sub);
 ...
 <div class="g2"><div class="phone"><div class="pbar"><b>ТАКТ · Grand Dance</b><span>Мангилик Ел 52</span></div><div class="pin"><div class="pcard"><small>Амина Сейткали · 7 лет</small><b>${esc(t.n)}</b><span>до ${dl(s.end)} · осталось ${subLeft(s)} занятия</span><div class="bar" style="margin-top:8px"><i class="g" style="--w:${pct(s.used,t.les)}%"></i></div></div>
...
SC.integr=()=>`<div class="hd"><div><h2>Kaspi, WhatsApp, Instagram, 2ГИС</h2><p>Честно про интеграции: что подключается напрямую, что через выписку, что руками. Kaspi для ИП интеграцию не даёт — поэтому строим вокруг ежедневной выписки и ссылок на оплату, а не ждём API.</p></div></div>
 ...
 ['Kaspi · ссылки на оплату','да','Бот отправляет родителю ссылку с суммой и назначением «абонемент A8 · Амина · Мангилик Ел». Родитель платит в приложении Kaspi — в выписке платёж приходит с назначением, разносится сам.','g'],
...
SC.migrate=()=>`<div class="hd"><div><h2>Переход с Умай и MyClass</h2><p>«MyClass нам рекламировали, мы купили — и в итоге это две разные системы, которые не коллаборируются друг с другом». Переход без остановки работы. Данные переносятся за выходные, неделю системы работают параллельно, потом старые отключаются. Тренеры и администраторы учатся за одну смену — экраны проще, чем то, что есть сейчас.</p></div></div>
 ...
 ['Неделя 4','Запуск в 3 филиалах','Астана Мангилик Ел, Алматы Розыбакиева, Шымкент. Тренеры отмечают с телефона, администраторы принимают оплаты, родители получают ссылки. Параллельно старые системы.',''],
...
SC.phone=()=>{const l=LES.find(x=>x.id==='L-901');const g=G_(l.grp);const stu=STU.filter(s=>s.grp==='G-01');
 ...
 <div class="g3"><div class="phone"><div class="pbar"><b>ТАКТ · тренер</b><span>Руслан Даулетов</span></div><div class="pin"><div class="pcard"><small>Сегодня · чт 24.09</small><b>17:00 · ${esc(g.n)}</b><span>${esc(g.hall)} · Мангилик Ел 52 · 12 учеников</span></div>
 ...
 <div class="phone"><div class="pbar"><b>ТАКТ · администратор</b><span>Дана · Мангилик Ел 52</span></div><div class="pin"><div class="pcard"><small>Сейчас в зале · 17:00 латина 5–7</small>${stu.map(s=>`<div class="kv"><span>${esc(s.n)}</span><b class="${s.st==='nopay'?'neg':s.st==='trial'?'':'pos'}">${s.st==='nopay'?'без оплаты · '+fmt(s.due):s.st==='trial'?'пробное':'ок · '+subLeft(s)+' ост.'}</b></div>`).join('')}<button class="bt p" style="width:100%;margin-top:8px" onclick="act('phone-confirm')">Подтвердить занятие</button></div>
