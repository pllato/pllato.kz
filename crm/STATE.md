...
1. **Кнопка "Позвонить" в карточке контакта и сделки** (Pack B следующего чата). В deals.js уже есть `data-crm-tab="calls"` (вкладка звонков в сделке).
2. **Green-API настройка**: UI готов (форма в Настройки → Интеграции). Нужны креды клиента + тест отправки сообщения через `/wa/send`.
3. **SMTP канал** (Pack 3): UI формы пока не делали.
4. **UTM Phase 3**: публичный `/api/lead` для лендинга aminamed.kz.
5. **Согласование заказа Stage 2**: FIFO списание из остатков (требует уточнения у клиента).
6. **Универсальная корзина**: soft delete с TTL 30 дней.
7. **Унификация login UI**: вынести `LOGIN_HTML` в общий модуль (сейчас дублирован в `auth_gate.js` и `views/login.js`).
8. **Передача `isAdmin` при set-password** (мелкий fix).
9. **Repsly интеграция** (ждёт API key от клиента).
10. **Проверить deprecated workers**: `pllato-crm-worker`, `pllato-cpb-worker` — удалить если не используются.
