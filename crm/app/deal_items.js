...
export const ORDER_STATUS_ARCHIVED = "archived";       // после проведения «Реализации в 1С»
...
/**
 * Заказы в архиве — завершённый цикл (после проведения «Реализации в 1С»).
 * Сортировка по времени архивации (последние сверху).
 */
export function listArchivedDealOrders() {
  return Store.list(DEALS)
    .filter((d) => d.orderStatus === ORDER_STATUS_ARCHIVED && !d.isDeleted)
    .sort((a, b) => (b.orderArchivedAt || 0) - (a.orderArchivedAt || 0));
}

/**
 * Отправить отгруженный заказ в архив. Делается после того, как по заказу
 * проведена «Реализация в 1С» — цикл закрыт. Возвращает обновлённую сделку.
 */
export function archiveDealOrder(dealId) {
  const deal = Store.get(DEALS, dealId);
  if (!deal) throw new Error("Сделка не найдена");
  if (deal.orderStatus !== ORDER_STATUS_SHIPPED) {
    throw new Error("В архив можно отправить только отгруженный заказ");
  }
  const me = currentEmployee();
  const now = Date.now();
  const updated = Store.update(DEALS, dealId, {
    ...
    orderArchivedAt: now,
    orderArchivedBy: me?.id || null,
    orderArchivedByName: me?.name || me?.email || "Сотрудник",
  });
  try {
    Store.create("deal_activities", {
      dealId, type: "order_archived",
      text: deal.oneCRealizationNumber
        ? `Заказ отправлен в архив (реализация в 1С № ${deal.oneCRealizationNumber})`
...
/** Вернуть заказ из архива обратно в «Отгружены». */
export function unarchiveDealOrder(dealId) {
  const deal = Store.get(DEALS, dealId);
  if (!deal) throw new Error("Сделка не найдена");
  if (deal.orderStatus !== ORDER_STATUS_ARCHIVED) {
    throw new Error("Вернуть из архива можно только архивный заказ");
  }
  const me = currentEmployee();
  const now = Date.now();
  const updated = Store.update(DEALS, dealId, {
    orderStatus: ORDER_STATUS_SHIPPED,
    orderArchivedAt: null,
  });
  try {
    Store.create("deal_activities", {
      dealId, type: "order_unarchived",
      text: `Заказ возвращён из архива: ${me?.name || me?.email || "сотрудник"}`,
      authorId: me?.id || null, ts: now,
    });
  } catch (e) { /* noop */ }
  return updated;
...
export function reconcileOrderStatuses() {
  ...
  for (const doc of docs) {
    ...
    if (!deal) continue;
    // Уже отгружен или в архиве — не трогаем (иначе архивный заказ вернётся в shipped).
    if (deal.orderStatus === ORDER_STATUS_SHIPPED || deal.orderStatus === ORDER_STATUS_ARCHIVED) continue;
    try {
...
function renderCrmFooterActions(deal, items) {
  ...
  } else if (status === ORDER_STATUS_ARCHIVED) {
    note = `<span class="dim-footer-note" style="font-size:12.5px;color:#64748b">📁 Заказ в архиве — цикл закрыт.</span>`;
  }
...
function renderWarehouseFooterActions(deal, items) {
  ...
  // После проведения «Реализации в 1С» появляется кнопка «В архив».
  if (status === ORDER_STATUS_SHIPPED) {
    ...
    const invoiceNote = hasInvoice
      ...
      : "";
    const archiveBtn = hasRealization
      ? `<button type="button" class="btn-primary btn-sm" data-deal-order-archive title="Реализация в 1С проведена — отправить заказ в архив">📁 В архив</button>`
      : "";
    const printCls = hasRealization ? "btn-ghost btn-sm" : "btn-primary";
    return `
      ${invoiceNote}
      ${realizationBtn}
      <button type="button" class="${printCls}" data-deal-order-print title="Открыть печатную форму З-2">📄 Открыть накладную${deal.orderInvoiceNumber ? ` № ${escapeHtml(deal.orderInvoiceNumber)}` : ""}</button>
      ${archiveBtn}
    `;
  }
  // В архиве: цикл закрыт. Печать накладной + возможность вернуть в «Отгружены».
  if (status === ORDER_STATUS_ARCHIVED) {
    const note = `<span class="dim-footer-note" style="font-size:12.5px;color:#64748b;margin-right:6px">📁 В архиве${deal.oneCRealizationNumber ? ` · реализация № ${escapeHtml(deal.oneCRealizationNumber)}` : ""}.</span>`;
    return `
      ${note}
      <button type="button" class="btn-ghost btn-sm" data-deal-order-unarchive title="Вернуть заказ в «Отгружены»">↩ Из архива</button>
      <button type="button" class="btn-ghost btn-sm" data-deal-order-print title="Открыть печатную форму З-2">📄 Открыть накладную</button>
    `;
  }
  return "";
}
...
function wireModalHandlers() {
  ...
  // Согласовать заказ (бронь). Предоплата → «Бронь · ждём оплату».
  // Постоплата/консигнация → оплату не ждём: сразу формируем накладную и
  // согласовываем на отгрузку (минуем колонку оплаты).
  root.querySelector("[data-deal-order-approve]")?.addEventListener("click", async (e) => {
    e.preventDefault();
    const dealNow = Store.get(DEALS, dealId);
    const scheme = getOrderPaymentScheme(dealNow);
    const msg = scheme === "prepay"
      ? "Согласовать заказ? Товар забронируется, выставится счёт — заказ перейдёт в «Бронь · ждём оплату»."
      : "Согласовать заказ? Оплату не ждём (постоплата/консигнация) — сразу сформируется накладная, заказ перейдёт в «Согласованы на отгрузку».";
  ...
  // Отправить в архив (после проведённой реализации в 1С).
  root.querySelector("[data-deal-order-archive]")?.addEventListener("click", (e) => {
    e.preventDefault();
    ...
    try {
      ...
      alert(err?.message || "Не удалось отправить в архив");
    }
  });
  // Вернуть из архива в «Отгружены».
  root.querySelector("[data-deal-order-unarchive]")?.addEventListener("click", (e) => {
    e.preventDefault();
    try {
