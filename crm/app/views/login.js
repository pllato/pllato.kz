// Pllato CRM — Login screen overlay.
// Рендерится в полный экран если нет активной session.

import { loginByEmail, changePassword, getSession } from "../auth.js";
    ...
    <div class="login-card">
      ...
      ${!hasAnyEmail ? `
        <div class="login-warn">
          ⚠ В системе пока нет ни одного сотрудника с паролем.
      ...
      <div class="login-footer">
        <small>Пароль установлен администратором при добавлении</small>
      </div>
    </div>
  `;
}
...
function renderChangePasswordHTML(employee) {
  return `
    <div class="login-card">
      <div class="login-brand">
        <h1>Смена пароля</h1>
        <p class="login-sub">Здравствуй, ${escapeHtml(employee.name || employee.email)}!<br>Введи новый пароль для продолжения.</p>
      </div>
...
function wireLogin(overlay, onSuccess) {
  ...
  // Google: при клике закрыть оверлей и попросить Google sign-in (если уже настроено в app.js)
  overlay.querySelector("#loginGoogleBtn")?.addEventListener("click", () => {
    overlay.remove();
    // Существующий Google OAuth flow продолжится сам
    // Если в app.js Google рендерится автоматически — он покажется
    // Если нет — пользователь увидит существующий UI входа
    location.reload();
  });
}
...
function wireChangePassword(overlay, employee, onSuccess) {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      const result = await changePassword(employee.id, "", newPw);
      if (!result.ok) {
        // Старый пароль уже был проверен при логине — пробуем напрямую через setEmployeePassword
        const { setEmployeePassword } = await import("../auth.js");
        await setEmployeePassword(employee.id, newPw);
        const { Store } = await import("../store.js");
        Store.update("employees", employee.id, { forcePasswordChange: false });
      }
