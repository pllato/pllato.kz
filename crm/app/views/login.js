...
export async function showLoginOverlay(onSuccess) {
  const existing = document.getElementById("loginOverlay");
  if (existing) existing.remove();
  const overlay = document.createElement("div");
  overlay.id = "loginOverlay";
  overlay.className = "login-overlay";
  overlay.innerHTML = renderLoginHTML(null);
  document.body.appendChild(overlay);
  wireLogin(overlay, onSuccess);

  // Асинхронно проверяем есть ли пароли в системе
  hasAnyPasswordsAsync().then((hasAny) => {
    const warn = overlay.querySelector(".login-warn");
    if (!hasAny && !warn) {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (result.forcePasswordChange) {
        ...
        wireChangePassword(overlay, result.employee, onSuccess); return;
      }
      overlay.remove();
      if (onSuccess) onSuccess(result.employee); else location.reload();
    } catch (e) {
...
function wireChangePassword(overlay, employee, onSuccess) {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      overlay.remove();
      if (onSuccess) onSuccess(employee); else location.reload();
    } catch (e) {
      errEl.textContent = e.message || "Ошибка"; errEl.hidden = false;
